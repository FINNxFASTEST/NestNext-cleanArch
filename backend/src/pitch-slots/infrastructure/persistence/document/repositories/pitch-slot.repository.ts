...
import { PitchSlotSchemaClass } from '../entities/pitch-slot.schema';
import { PitchSlotRepository } from '../../pitch-slot.repository';
import { PitchSlot } from '../../../../domain/pitch-slot';
import { PitchSlotMapper } from '../mappers/pitch-slot.mapper';
...
export class PitchSlotDocumentRepository implements PitchSlotRepository {
  ...
  async reserve(data: PitchSlot[]): Promise<PitchSlot[]> {
    // insertMany with ordered:true stops at first duplicate and rolls back
    // via our compensation logic in BookingsService (on E11000).
